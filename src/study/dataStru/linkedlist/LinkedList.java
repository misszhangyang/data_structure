...
class SingleLinkedList {
	...
	public void addBySquence(Node node) {
		...
		while (true) {
			...
			if (tmpNode.next.number < node.number) {// ????????ß÷??????????no?????????????????ß≥???????????ßµ???¶À??????????????
				break;
			}
		...
		if (flag) {
			System.out.println("????????????????ßµ?" + node.number);
		} else {
			// ?????????????????,tmpNode??????ó®tmpNode.next?????
	...
	public void updateNode(Node node) {
		...
		if (flag) {
			System.out.println("??????????????¶À?????????????" + node.number);
	...
	public void delNode(int number) {
		...
		if (flag) {
			// ????????????????¶À????????????????tmpNode,??????????????????next???tmpNode.next.next????
